...
export default {
  ...
  created() {
    axios.get(`/api/member/${this.$route.params.id}`).then(res => {
      this.form = res.data.data
    }).catch(error => console.log(error))
  },
  methods: {
    onSubmit() {
      axios.patch(`/api/member/${this.$route.params.id}`, this.form).then(res => {
        window.location = "/master/anggota/detail/"+this.$route.params.id;
      }).catch(error => {
        console.log(error);
